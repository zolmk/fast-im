...
import com.feiyu.route.config.ZkRouteConfiguration;
...
public class ZkServiceFinder implements ServiceFinder {
    ...
    public ZkServiceFinder(ZkRouteConfiguration configuration) {
        String rootPath = configuration.getRootPath();
        String connectString = configuration.getConnectString();
        this.serviceId = configuration.getNodeId();
        this.serviceIp = configuration.getNodeIp();
    ...
    public void registerServer() {
        try {
            this.client.create().withMode(CreateMode.EPHEMERAL).forPath(toServerPath(serviceId), this.serviceIp.getBytes(StandardCharsets.UTF_8));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
